// #ifdef APP-PLUS
import componentConfig from "/lib/update/componentConfig"
const platform = uni.getSystemInfoSync().platform;
// 主颜色
const $mainColor = componentConfig.appUpdateColor ? componentConfig.appUpdateColor : "FF5B78";
// 弹窗图标url
const $iconUrl = componentConfig.appUpdateIcon ? componentConfig.appUpdateIcon : "/static/img/ic_ar.png";
...
export default function(isPrompt = false) {
	getCurrentNo(versionInfo => {
		...
		componentConfig.getServerNo(versionInfo, isPrompt, res => {
			...
			// if(!compareVersions(res.versionCode, versionInfo.versionName)) {
			// if(+res.versionCode <= +versionInfo.versionCode) {
	/*		if(
				(/\.wgt$/i.test(res.downloadUrl) && +res.versionCode <= +versionInfo.versionCode) ||
				!compareVersions(res.versionCode, versionInfo.versionName)
			) {
				return false
			}*/
			if (res.updateType == "forcibly" || res.updateType == "silent") {
				if (res.downloadUrl.indexOf(".wgt") > -1) {
					getDownload(res);
				} else if(res.downloadUrl.indexOf(".html") > -1){
					plus.runtime.openURL(res.downloadUrl);
				} else {
					if (platform == "android" && res.downloadUrl.indexOf(".apk") > -1) {
						getDownload(res);
					} else {
						plus.runtime.openURL(res.downloadUrl);
					}
